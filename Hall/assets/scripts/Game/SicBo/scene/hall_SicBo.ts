...
export default class hall extends cc.Component {
	...
	m_jetton = [0,0,0,0]   //自己下注筹码数据
	m_wheeljetton = [0,0,0,0]   //自己下注筹码数据
	...
	protected start(): void {
		...
		let game_path =`Game/SicBo/resources/loadRes/img`
		cc.resources.loadDir(game_path, cc.Asset, (completedCount, totalCount, item)=>{
	...
	fucCoCosgetMsg(e){
		...
		if (e.data.type == "userbalance") {
			...
			for (let index = 0; index < e.data.data.Data1.item.length; index++) {
				pair = e.data.data.Data1.item[index].betting_issuseNo.slice(4)
				if (inningCount == Number(pair) && e.data.data.Data1.item[index].Winning_amount) {
					self.m_GameStatus.m_WinGold += Number(e.data.data.Data1.item[index].Winning_amount)
				}
			}
		}
		//这是断线重连
			...
			for (let index = 0; index < e.data.data.Data.item.length; index++) {
				...
				let pair = Item_data.IssueNo.slice(4)  //qianm
	   ...
	   //这是获取开奖记录  只能调用一次
			...
			let Item_data = e.data.data.Data.item[0]
			//游戏局数
			let betnumber= ["Big","Small","Odd","Even"]
	...
	initChangeImage(){
		...
		for (let index = 0; index < 4; index++) {
			let sp = this.middleRoot.getChildByName('Oddslayer').getChildByName('sp_'+(index+1)) 
	...
	//动画控制器
	initAction(){
		this.m_AnimationComponent  = this.node.getChildByName("AnimtionNode").getComponent("AnimationControl_SicBo")
		if (!GameInstInfo.getinstance().m_Disconnect) {
			...
			if (index == 2) {
				...
				if (!this.m_GameExplainComponent) {
					...
					cc.resources.load(GameInstInfo.getinstance().getSubgameByGameID(GameInstInfo.getinstance().m_SelectGameID).pathOfGameExplainPrefab, cc.Prefab, (err, prefab) => {
						...
						self.m_GameExplainComponent =  instance.getComponent("GameExplain_SicBo")
	...
	initMiddleRoot(){
		...
		for (let index = 0; index < 4; index++) {
			...
			this.m_AllDonwButPos.push(this.middleRoot.getChildByName('Pos_'+(index+1)))
		}
	...
	initComponent(){
		//初始化开将记录
		this.m_RecordsComponent =  this.node.getChildByName("nRecordLayout").getComponent("RecordLayout_SicBo")
		//初始化历史记录
		this.m_RecordVerComponent=  this.node.getChildByName("nRecordRank").getComponent("RecordRank_SicBo")
		this.m_RecordVerComponent.m_parent = this
		this.m_BettingRecordComponent =  this.node.getChildByName("nBettingRecord").getComponent("BettingRecord_SicBo")
		//this.m_GameExplainComponent =  this.node.getChildByName("nGameExplain").getComponent("GameExplain_SicBo")
	}
	...
	fucPlaceBet(index){
		...
		let n_parent = this.m_AllDonwButPos[index]
		nchip.active = true
		...
		posx = pos.x
		posy = pos.y
		//GameInfo.getinstance().pointAtRotation({posx,posy},n_parent.angle)
	...
	fucUserPlaceBet(index,_number){
		...
		nchip.stopAllActions()
		let n_parentMoney = this.m_AllDonwButNode[index]
		nchip.runAction(cc.sequence(cc.show(),cc.moveTo(n_Time,cc.v2(posx,posy)).easing(cc.easeCubicActionOut()),cc.callFunc(()=>{
			SoundMgr.palyChipSound()
			//这里统计下注额度
			self.fucUpArerMoney(n_parentMoney,self.m_GameStatus.betsectionGold[index])
		})))
	...
	fucCountdown(){
		...
		if (this.m_GameStatus.gamestatus == 1) {
			...
			if (this.m_GameStatus.gameDownChipTime <= 0 ) {
				...
				//NPC动画
				this.m_AnimationComponent.fucPlayNPC(1)
	...
	//游戏结果
	fucCardsOpen(){
		let  s_soundStr = ["dices_","small_","big_","triple_"]
		this.m_AnimationComponent.fucPlayOpen(this.m_GameStatus.TouziPonits,function(index){
			...
			SoundMgr.palyFishCrabsSound(s_soundStr[0]+this.m_GameStatus.TouziPonits[0])
			this.scheduleOnce(function(){
				SoundMgr.palyFishCrabsSound(s_soundStr[0]+this.m_GameStatus.TouziPonits[1])
			},0.5)

			this.scheduleOnce(function(){
				SoundMgr.palyFishCrabsSound(s_soundStr[0]+this.m_GameStatus.TouziPonits[2])
			},0.8)
			this.scheduleOnce(function(){
				//播放大小
				let n_count = this.m_GameStatus.TouziPonits[0]+this.m_GameStatus.TouziPonits[1]+this.m_GameStatus.TouziPonits[2]
				if (this.m_GameStatus.TouziPonits[0] == this.m_GameStatus.TouziPonits[2]  && this.m_GameStatus.TouziPonits[1] == this.m_GameStatus.TouziPonits[2]) {
					SoundMgr.palyFishCrabsSound(s_soundStr[3]+n_count)
				}else{
					if (n_count > 10) {
						SoundMgr.palyFishCrabsSound(s_soundStr[2]+n_count)
					}else{
						SoundMgr.palyFishCrabsSound(s_soundStr[1]+n_count)
					}
				}
			},1.3)
	...
	fucSetRate(){
		let dtp = ['1','1','2','2']
		for (let index = 0; index < 4; index++) {
			let str_Rate_lab = this.middleRoot.getChildByName('Oddslayer').getChildByName("str_"+(index+1))
			GameInstInfo.getinstance().m_GameRate.findIndex((elem: any) => {
				if (elem.PlayCode == dtp[index]){
	...
	fucMoveChipDesk(n_needMovechip){
		...
		let z_parentpos  = 0
		let n_parent:cc.Node = this.m_AllDonwButPos[n_result[z_parentpos]];
		...
		for (let index = 0; index < n_needMovechip.length; index++) {
			...
			if (index  >= middleIndex) {
				...
				z_parentpos += 1
				n_parent = this.m_AllDonwButPos[n_result[z_parentpos]];
			}
			let posSize = n_parent.getContentSize()
			let posx = n_parent.x  - posSize.width/2  + Math.random()*(posSize.width-nchip.getContentSize().width/2)
			let posy = n_parent.y  - posSize.height/2 + nchip.getContentSize().height/2 + Math.random()*(posSize.height-nchip.getContentSize().height/2)
	...
	fucPlayerWinNumber(){
		...
		function fucMoveAction(Target:any,_count:number,angle:number,pos:cc.Vec3,TargetPos:cc.Vec2,callfunc= null){
			...
			winLab.angle = angle;
			winLab.runAction(cc.sequence(cc.moveBy(1.5,TargetPos),cc.callFunc(()=>{
				if (callfunc) {
					callfunc()
				}
			}),cc.removeSelf()))
		...
		fucMoveAction(winAction,myWin,0,new cc.Vec3(0,20,1),cc.v2(0,50),function(){
			if (myWin < 10) {
				self.m_AnimationComponent.fucPlayNPC(3)
			}
		})
		//在线玩家
	...
	fucMoveAllChip(){
		...
		let WinNum = 0
		let reparation = [15.84,1.98,1.98,15.84]
		let n_result = this.fucGetWinTarget()
