...
export default class hall extends cc.Component {
	...
	m_jetton = [0,0,0,0,0,0,0,0]   //自己下注筹码数据
	m_wheeljetton = [0,0,0,0,0,0,0,0]   //自己下注筹码数据
	...
	m_GameStatus = {
		...
		TouziPonits:0,
		//区间
		betsectionGold:[0,0,0,0,0,0,0,0],
		//游戏当前状态
	...
	fucCoCosgetMsg(e){
		...
		if (e.data.type == "userbalance") {
			...
			self.m_wheeljetton = [0,0,0,0,0,0,0,0,]
		}
		...
		//这是断线重连
			...
			for (let index = 0; index < e.data.data.Data.item.length; index++) {
				...
				if (self.m_GameStatus.inningCount == Number(pair)) {
					bGetdata = true
					//获取开奖结果
					self.m_GameStatus.TouziPonits= Number(Item_data.LotteryOpen)
				}
			}
			if (bGetdata) {
	   ...
	   //这是获取开奖记录  只能调用一次
			...
			if (self.m_GameStatus.gameDownChipTime < gameConfig.Bet_Reward_Time) {
				if (self.m_GameStatus.gamestatus == 1) {
					...
					let  b = [0.1,0.7,0.7,0.1,0.2,0.7,0.7,0.2];
			...
			*/
			...
			let Item_data = e.data.data.Data.item[0]
			//游戏局数
			let betnumber= ["0" , "even" ,  "odd"  ,  "4" , "1" , "small"  ,  "big"   , "3"]
	...
	fucServerMessige(){
		//获取赔率
		this.fucPostMessage({type:"getrebate",param:{method:"post",url:'getrebate',data:{LotteryType:"cd"} } })
	...
	initChangeImage(){
		...
		for (let index = 0; index < array.length; index++) {
			let sp = this.middleRoot.getChildByName('Down_'+(array[index])).getChildByName('sp_Exp') 
	...
	//动画控制器
	initAction(){
		this.m_AnimationComponent  = this.node.getChildByName("AnimtionNode").getComponent("AnimationControl_Dish")
			...
			if (index == 2) {
				...
				if (!this.m_GameExplainComponent) {
					...
					cc.resources.load(GameInstInfo.getinstance().getSubgameByGameID(GameInstInfo.getinstance().m_SelectGameID).pathOfGameExplainPrefab, cc.Prefab, (err, prefab) => {
						...
						self.m_GameExplainComponent =  instance.getComponent("GameExplain_Dish")
				...
				this.fucPostMessage({type:"getbetrecord", param:{method:"post", url:'getbetrecord',data:{lottery_code:this.m_GameStatus.mlottery_code,LotteryType:"cd"} } })
			}
		}.bind(this))
	...
	initMiddleRoot(){
		...
		for (let index = 0; index < 8; index++) {
			let down_but = this.middleRoot.getChildByName('Down_'+(index+1)).getChildByName("but")
	...
	initComponent(){
		//初始化开将记录
		this.m_RecordsComponent =  this.node.getChildByName("nRecordLayout").getComponent("RecordLayout_Dish")
		//初始化历史记录
		this.m_RecordVerComponent=  this.node.getChildByName("nRecordRank").getComponent("RecordRank_Dish")
	...
	fucRevocation(){
		...
		this.m_wheeljetton= [0,0,0,0,0,0,0,0]
	...
	fucUpMyAmount(){
		...
		for (let index = 0; index < this.m_AllDonwButNode.length; index++) {
			element = this.m_AllDonwButNode[index];
			str_lab = element.getChildByName('userchipbg').getChildByName('str_Label')
			...
			if (this.m_GameStatus.gameDownChipTime > 3 && this.m_GameStatus.gamestatus == 1) {
				...
				for (let index = 0; index < count; index++) {
					...
					this.node.runAction(cc.sequence(cc.delayTime(n_DeleyTiame),cc.callFunc(()=>{
						...
						this.fucUserPlaceBet(_index,ChipNumber[Math.round(Math.random()*(ChipNumber.length-1))])
					})))
				}
			}
	...
	fucSetRate(){
		...
		for (let index = 0; index < 8; index++) {
			let str_Rate_lab = this.middleRoot.getChildByName('Down_'+(index+1)).getChildByName("str_Rate_lab")
	...
	//TouziPonits 两个子   判断是不是红子
	fucGetWinTarget(){
		//根据红的数目判断哪几个中奖
		let n_result =[ [0,1,5],     //4白
						[2,4,5],	 //1红3白
						[1],		 //2红2白
						[2,6,7],	 //3红1白
						[1,3,6]] 	 //4红
	...
	fucGameEndAction(){
		...
		for (let index = 0; index < n_result.length; index++) {
			//显示亮底
			desk_Hight = this.m_AllDonwButNode[n_result[index]].getChildByName('desk_Hight')
	...
	fucClear(){
		...
		this.m_GameStatus.betsectionGold=[0,0,0,0,0,0,0,0]

		this.m_jetton= [0,0,0,0,0,0,0,0]
		this.m_wheeljetton= [0,0,0,0,0,0,0,0]
		this.fucUpMyAmount()
