...
export default class BettingRecord extends cc.Component {
	...
	@property({type:[cc.SpriteFrame],displayName:"6个元素"})
	...
	fucSetItemInfo(data){
		...
		//6个下注区域   庄重复
		//["Banker1", "Player1", "Banker2", "Player2", "Banker3", "Player3"]  
		let betting_number = GameInstInfo.getinstance().getSubgameByGameID(GameInstInfo.getinstance().m_SelectGameID).gameArea
		let  result = Item.getChildByName("jieguo")
		if (data.BetNum == betting_number[0]) {
			Item.getChildByName("mb_tx_puke_1").active = true
			result.getComponent(cc.Sprite).spriteFrame = this.m_SpriteMode[0]
		}
		else if (data.BetNum == betting_number[1]) {
			result.getComponent(cc.Sprite).spriteFrame = this.m_SpriteMode[1]
		}
		else if (data.BetNum == betting_number[3]) {
			result.getComponent(cc.Sprite).spriteFrame = this.m_SpriteMode[2]
		}
		else if (data.BetNum == betting_number[5]) {
			result.getComponent(cc.Sprite).spriteFrame = this.m_SpriteMode[3]
		}
		else if (data.BetNum == betting_number[2]) {
			result.getComponent(cc.Sprite).spriteFrame = this.m_SpriteMode[0]
			Item.getChildByName("mb_tx_puke_2").active = true
		}
		else if (data.BetNum == betting_number[4]) {
			result.getComponent(cc.Sprite).spriteFrame = this.m_SpriteMode[0]
			Item.getChildByName("mb_tx_puke_3").active = true
		}

		GameInstInfo.getinstance().fucChangeImage(result)
		Item.getChildByName("name").getComponent(cc.Label).string = GameEngine.m_services.i18nSrv.getI18nString("百人牛牛")  
