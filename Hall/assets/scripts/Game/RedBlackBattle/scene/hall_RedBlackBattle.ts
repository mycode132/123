...
import Compare from "../ControlScript/Compare";
import GameInfo from "../ControlScript/GameInfo_RedBlack";
...
export default class hall extends cc.Component {
	...
	m_GameStatus = {
		...
		betAllGold:0,
		//游戏结果
		GameCardsData: [13, 7, 4, 10, 1, 9, 1, 5, 1, 12, 4, 4, 4, 4, 3, 4, 2, 1, 3, 2],
		//区间
		betsectionGold:[],
		//游戏当前状态
	...
	initChangeImage(){
		...
		//赔率上8个图片需要动态更改 NNode
		let n_oddsNode = this.middleRoot.getChildByName('Down_node').getChildByName('Down_3').getChildByName('NNode') 
	...
	//动画控制器
	initAction(){
		this.m_AnimationComponent  = this.node.getChildByName("AnimtionNode").getComponent("AnimationControl_RedBlack")
		if (!GameInstInfo.getinstance().m_Disconnect) {
			...
			if (index == 2) {
				...
				if (!this.m_GameExplainComponent) {
					...
					cc.resources.load(GameInstInfo.getinstance().getSubgameByGameID(GameInstInfo.getinstance().m_SelectGameID).pathOfGameExplainPrefab, cc.Prefab, (err, prefab) => {
						...
						self.m_GameExplainComponent =  instance.getComponent("GameExplain_RedBlack")
	...
	initComponent(){
		//初始化开将记录
		this.m_RecordsComponent =  this.node.getChildByName("nRecordLayout").getComponent("RecordLayout_RedBlack")
		//初始化历史记录
		this.m_RecordVerComponent=  this.node.getChildByName("nRecordRank").getComponent("RecordRank_RedBlack")
		this.m_RecordVerComponent.m_parent = this

		this.m_BettingRecordComponent =  this.node.getChildByName("nBettingRecord").getComponent("BettingRecord_RedBlack")
		//this.m_GameExplainComponent =  this.node.getChildByName("nGameExplain").getComponent("GameExplain_RedBlack")
	}
	...
	fucDisconne(_type,index = 0){
		...
		element.setScale(0)
		element.active = true
		let n_CardsPk1Component =element.getComponent("CardsMode_RedBlack")
	...
	fucPlayerSound(){
		...
		//红   0：赢     1：输
		//  l:2   W:4
		let index = this.m_GameStatus.m_nWinLose == 0 ? 4 : 2
		if (index == 4) {
			SoundMgr.palyGamePublicSound("female_attack")
			this.m_AnimationComponent.fucPlayOpen(4,function(index){
				SoundMgr.palyGamePublicSound("female_win")
				this.scheduleOnce(function(){
					SoundMgr.palyGamePublicSound("male_lose")
				},2.0)
			}.bind(this),true)
			
			this.scheduleOnce(function(){
				SoundMgr.palyGamePublicSound("hurt")
				this.m_AnimationComponent.fucPlayOpen1(2,function(index){
					
				}.bind(this),false)
			},1.5)
		}else{
			//延时1秒
			SoundMgr.palyGamePublicSound("male_attack")
			this.scheduleOnce(function(){
				SoundMgr.palyGamePublicSound("hurt")
				this.m_AnimationComponent.fucPlayOpen(2,function(index){
					SoundMgr.palyGamePublicSound("male_win")
					this.scheduleOnce(function(){
						SoundMgr.palyGamePublicSound("female_lose")
					},0.5)

				}.bind(this),true)
			},1.0)
			this.m_AnimationComponent.fucPlayOpen1(4,function(index){
	...
	fucPipcount(){
		...
		this.m_GameStatus.Cardresult[0] = this.m_Compare.getType(cards)
		cards = []
		for (let index = 5; index < 10; index++) {
			cards[index-5] = this.m_GameStatus.GameCardsData[index];
		}
		this.m_GameStatus.Cardresult[1] = this.m_Compare.getType(cards)
	}

	//扑克比大小
	fucGetWinTarget(){
		if (this.m_GameStatus.Cardresult[0] == this.m_GameStatus.Cardresult[1]) {
			let  cards_1 = []
			for (let index = 0; index < 5; index++) {
			...
			if ( this.m_GameStatus.Cardresult[0]== 0) {
				return this.m_Compare.fucBankCompare(cards_1,cards_2)
			}
			//对子  两对    找出最大对子
			if((this.m_GameStatus.Cardresult[0] == this.m_Compare.PaiType.YD-1  ||  this.m_GameStatus.Cardresult[0] <= this.m_Compare.PaiType.ED -1
				|| this.m_GameStatus.Cardresult[0] <= this.m_Compare.PaiType.ST -1 || this.m_GameStatus.Cardresult[0] <= this.m_Compare.PaiType.TZ -1 
				|| this.m_GameStatus.Cardresult[0] <= this.m_Compare.PaiType.HL -1)){
				return this.m_Compare.fucCardRresult(cards_1,cards_2)
			}
			//顺子 同花顺 比较 
			if(this.m_GameStatus.Cardresult[0] > this.m_Compare.PaiType.SZ -1  ||  this.m_GameStatus.Cardresult[0] <= this.m_Compare.PaiType.THS -1
				||  this.m_GameStatus.Cardresult[0] <= this.m_Compare.PaiType.HJTHS -1  ){
				return this.m_Compare.fucShunZiCompare(cards_1,cards_2)
			}
			//不成牌型     同花  散牌比较最大牌
			return this.m_Compare.fucBankCompare(cards_1,cards_2)
		}
		return (this.m_GameStatus.Cardresult[0] > this.m_GameStatus.Cardresult[1] ) ? 0 : 1
	}
	...
	fucCountdown(){
		...
		if (this.m_GameStatus.gamestatus == 1) {
			...
			if (this.m_GameStatus.gameDownChipTime <= 0 ) {
				...
				this.m_AnimationComponent.fucPlayTouzi()
		...
		//开奖 8秒状态   翻牌   等2秒   播放龙虎动画   移动筹码到庄  移动筹码到赢属性   移动筹码到玩家   结束
			...
			if (this.m_GameStatus.gameDownChipTime == 11) {
				...
				//开牌
				this.fucCardsOpen(0,5)
				this.fucCardsOpen(5,10)
				this.fucPipcount()
				//这个地方请求自己金币变化
				this.fucPostMessage({type:"userbalance",param:{ method:"post",url:'userbalance',data:{}}})
			}
	...
	fucMoveChipDesk(n_needMovechip){
		...
		let n_parent:cc.Node = this.m_AllDonwButNode[this.m_GameStatus.m_nWinLose];
		for (let index = 0; index < n_needMovechip.length; index++) {
			let  nchip = n_needMovechip[index];
	...
	fucPlayerWinNumber(){
		...
		//玩家自己   赔率  需要修改
		let myWin = this.m_jetton[this.m_GameStatus.m_nWinLose ] 
		GameInstInfo.getinstance().m_GameRate.findIndex((elem: any) => {if (elem.PlayCode == "dtp1"){
			myWin = this.m_jetton[this.m_GameStatus.m_nWinLose ] * Number(elem.Bonus[this.m_GameStatus.m_nWinLose])
		}});
	...
	//游戏结束  动画控制
	fucGameEndAction(){
		const n_parent =this.m_AllDonwButNode[this.m_GameStatus.m_nWinLose]
		//显示亮底
