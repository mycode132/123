...
export default class BettingRecord extends cc.Component {
	...
	fucUpdata(data){
		...
		for (let index = 0; index < data.length; index++) {
			element = data[index];
			info = JSON.parse(JSON.stringify(this.m_BettingInfo))
	...
	fucSetItemInfo(data){
		...
		let lab_issuseNo = Item.getChildByName("lab_issuseNo")
		...
		//已中奖不予显示
		//BetNum   Tiger    BetNum: "Tie"  BetNum: "Dragon"
		let node = null
		if (data.BetNum == "Tiger" || data.BetNum =="虎") {
			node = Item.getChildByName("Tiger")
		}else if(data.BetNum == "Tie"|| data.BetNum =="和"){
			node = Item.getChildByName("Tie")
		}else if(data.BetNum == "Dragon" ||  data.BetNum =="龙" ){
			node = Item.getChildByName("Dragon")
		}
		node.active = true
		GameInstInfo.getinstance().fucChangeImage(node)
