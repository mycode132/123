...
class GIF {
    ...
    public getSpriteFrame (index) {
        return new Promise(async (resolve) => {
            if (this._spriteFrames[index]) resolve(this._spriteFrames[index]);

            let spriteFrames = await this.decodeFrame2Texture(this._info.frames[index], index)
    ...
    private decodeFrame2Texture (frame, index) {
        return new Promise((resolve) => {
            ...
            if (CC_JSB) {
                ...
                resolve(this._spriteFrames[index])
            } else {
                // 微信小程序平台
